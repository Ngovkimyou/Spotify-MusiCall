        ...
        if self.sp:
            ...
            if current_playback and current_playback["is_playing"]:
                ...
                return {
                    ...
                    "spotify_url": song["external_urls"]["spotify"],  # Link to song on Spotify
                }
        return None
    ...
    def get_recent_tracks(self):
        if self.sp:
            ...
            for item in recent_tracks["items"]:
                ...
                album_image = item["track"]["album"]["images"][0]["url"]  # Get album cover image
...
def recent_tracks():
    if 'user_id' in session:
        ...
        for track in tracks:
            track["played_at"] = datetime.strptime(track["played_at"], "%Y-%m-%dT%H:%M:%S.%fZ").strftime("%Y-%m-%d %H:%M:%S")
